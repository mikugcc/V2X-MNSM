...
from v2xmnsm.msgs import DENM, LANE_CHANGING, json_to_package
...
class CarCtrller(SumoStepListener): 
    ...
    def set_lane_change_timer(self, to_lane: int, time: int) -> None:
        if self.__vlc.next_lane is not None: return None
...
class AutoCar01Ctrller(CarCtrller): 
    ...
    def __handle_in_denm(self) -> None:
        for _ in range(self.__vlc.mesh_packages.qsize()): 
            ...
            if package.body.location.lane != self.__vlc.lane: continue
...
class AutoCar02Ctrller(CarCtrller): 
    ...
    def __handle_lane_changer(self) -> None: 
        threat_vehicles = self.detact_threat_vlcs()
        for t_vlcs, t_position in threat_vehicles.items(): 
...
class NormalCarCtrller(CarCtrller): 
    ...
    @SumoStepListener.Substeps(priority=9)
    def __change_lane(self) -> None: 
        if 100 < self.__vlc.distance < 102: 
            self.set_lane_change_timer(self.__cur_lane^1, time=5)
        return None

    @SumoStepListener.Substeps(priority=9)
    def __change_lane_after_delay(self) -> None:
        if self.is_timer_alart(self.__cur_lane ^ 1): 
            self.__cur_lane ^= 1 
        return None
